...
template<spec> \
struct pusher<type>\
{ static void push(std::shared_ptr<lua_State> state, type param){ pushcommand; \
    }                                                                          \
    \
}                                                                       \
    ;                                                                          \
...
template<spec> \
struct popper<type>\
{ static type pop(std::shared_ptr<lua_State> state){ popcommand;               \
    lua_pop(state.get(), 1);                                                   \
    return res;                                                                \
    }                                                                          \
    \
...
struct pusher
{
    ...
    {
        static_assert(sizeof(T) == 0, "parameter types other than signed or unsigned char, short, int, long, "
                                      "std::string, bool, pointers, LuaCoroutine, LuaUserdata, LuaTable or LuaFunction "
                                      "not available");
    }
};
...
struct popper
{
    ...
    {
        static_assert(sizeof(T) == 0, "returns types other than signed or unsigned char, short, int, long, "
                                      "std::string, bool, pointers, LuaCoroutine, LuaUserdata, LuaTable or LuaFunction "
                                      "not available");
    }
};
...
{
    static void push(std::shared_ptr<lua_State> state, const char* param) { lua_pushstring(state.get(), param); }
};
...
// strings!
DEFINE_TYPE_TEMPLATE_FOR(const char*,
                         ,
...
DEFINE_TYPE_TEMPLATE_FOR(LuaLightUserdata<TYPE>,
                         typename TYPE,
                         param.PushToStack(state.get()),
                         LuaLightUserdata<TYPE> res(state, -1))
DEFINE_TYPE_TEMPLATE_FOR(LuaUserdata<TYPE>,
                         typename TYPE,
                         param.PushToStack(state.get()),
                         LuaUserdata<TYPE> res(state, -1))
DEFINE_TYPE_TEMPLATE_FOR(LuaFunction<SIG>,
                         typename SIG,
                         param.PushToStack(state.get()),
...
// pointers
DEFINE_TYPE_TEMPLATE_FOR(TYPE*,
                         typename TYPE,
                         lua_pushlightuserdata(state.get(), (void*)param),
                         TYPE* res = LuaLightUserdata<TYPE>(state, -1).GetPointer())
...
DEFINE_TYPE_TEMPLATE_FOR(uint16_t,
                         ,
                         lua_pushunsigned(state.get(), param),
                         uint16_t res = lua_tounsigned(state.get(), -1))
DEFINE_TYPE_TEMPLATE_FOR(uint32_t,
                         ,
                         lua_pushunsigned(state.get(), param),
                         uint32_t res = lua_tounsigned(state.get(), -1))
DEFINE_TYPE_TEMPLATE_FOR(uint64_t,
                         ,
                         lua_pushunsigned(state.get(), param),
                         uint64_t res = lua_tounsigned(state.get(), -1))
