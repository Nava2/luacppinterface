...
LuaReference::LuaReference(const LuaReference& other) { *this = other; }
...
LuaReference::~LuaReference() { luaL_unref(state.get(), LUA_REGISTRYINDEX, ref); }

void LuaReference::PushToStack(lua_State* currentState) const { lua_rawgeti(currentState, LUA_REGISTRYINDEX, ref); }
...
std::shared_ptr<lua_State> LuaReference::GetState() { return state; }
