...
public:
    ...
    {
        return CreateUserdata<TYPE>(data,
                                    [](TYPE* data)
                                    {
                                        delete data;
                                    });
    ...
    {
        typename LuaUserdata<TYPE>::UserdataWrapper* wrap
            = (typename LuaUserdata<TYPE>::UserdataWrapper*)lua_newuserdata(
                state.get(), sizeof(typename LuaUserdata<TYPE>::UserdataWrapper));
    ...
    {
        return internalCreateFunction(std::shared_ptr<std::function<SIG>>(new std::function<SIG>(func)),
                                      lua_normalFunction<SIG>);
    }
    ...
    {
        return internalCreateFunction(std::shared_ptr<std::function<SIG>>(new std::function<SIG>(func)),
                                      lua_yieldingFunction<SIG>);
    }

    void CollectGarbage() { lua_gc(state.get(), LUA_GCCOLLECT, 0); }
};
